...
function addReview(bookId,reviewObj){
    ...
    if(book.reviews)book.reviews.push(reviewObj)
    else book.reviews = [reviewObj];
    storageService.store(BOOKS_KEY, booksDB);
    return Promise.resolve(JSON.parse(JSON.stringify(book.reviews)))
}

function query() {
    loadBooks();
    return Promise.resolve(JSON.parse(JSON.stringify(booksDB)));
}

function getBookById(bookId){
    loadBooks();
    let corrInx
    const corrBook = JSON.parse(JSON.stringify(booksDB.find((book,inx)=>{if(book.id===bookId){corrInx=inx;return true}}))) 
    let nextInx = corrInx + 1
    if(nextInx===booksDB.length) nextInx=0;
    corrBook.next = booksDB[nextInx].id;
    let prevInx = corrInx - 1
    if(prevInx<0)prevInx=booksDB.length-1;
    corrBook.prev = booksDB[prevInx].id;

    return Promise.resolve(corrBook);
}
